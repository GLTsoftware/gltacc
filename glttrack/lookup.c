...
double 	radian;
/*
double LONGITUDE_HOURS = -7.174712960;
        double LATITUDE_DEGREES =   34.75083333;
...
double        LONGITUDE_HOURS  = -7.174712960;
double        LATITUDE_DEGREES  = 34.75083333;
double        LONGITUDE_DEGREES  = -107.6206944;
double        SINLAT  = 0.570008713;
double        COSLAT  = 0.8215638648;
double        LONGRAD  = -1.878335461;
double        HEIGHT_M  = 2124.0;
	  ...
	  struct  smapoptOption optionsTable[] = {
		...
		{"time",'t',SMAPOPT_ARG_STRING,&timeString,'t',
			"date time: DD MonthName YYYY HH:MM:SS."},
                ...
                "Lookup optical catalog instead of radio catalog."},
		{"when",'w',SMAPOPT_ARG_NONE,0,'w',
                "verbose flag for rise/set/transit times"},
                SMAPOPT_AUTOHELP
                {NULL,0,0,NULL,0},
	...
	/* calculating julian day */
	   ...
	   tjd=tjd_upper+tjd_lower;
		...
		et2utc_c (  et , "C", 8, UTCLEN, utcstr );
                sscanf(utcstr,"%s %s %s %02d:%02d:%lf",
			junk,junk,junk,&utchour, &utcmin,&utcsec);
	...
	if (nplanet == 0) {
	 ...
	 phi=radian*LATITUDE_DEGREES;
        ...
        hms(&utcTransit,&utcTransitH,&utcTransitM,&utcTransitS,&dummy);
             ...
             if((Planet.number==501)||(Planet.number==502)
			||(Planet.number==503)||(Planet.number==504)) {
...
/*
3 aug 2009; these next two if statements were causing the first source
to be skipped.
*/
	if(optical==0) {
