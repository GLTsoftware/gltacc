...
#define MUTEX_LOCK(mutex_addr, function_name, host) {     \
  int ssssl;                                              \
  dsm_printf("%s(%s): locking mutex " #mutex_addr "\n",      \
          function_name, host);                           \
  ...
  if(ssssl != 0) {                                        \
    ...
    dsm_printf("%s(%s): locked mutex " #mutex_addr "\n",     \
            function_name, host);                         \
  }                                                       \
}
...
#define MUTEX_UNLOCK(mutex_addr, function_name, host) {     \
  int ssssu;                                                \
  dsm_printf("%s(%s): unlocking mutex " #mutex_addr "\n",      \
          function_name, host);                             \
  ...
  if(ssssu != 0) {                                          \
    ...
    dsm_printf("%s(%s): unlocked mutex " #mutex_addr "\n",     \
            function_name, host);                           \
  }                                                         \
}
