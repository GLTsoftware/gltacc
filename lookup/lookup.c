...
double  pi ;
double 	radian;
double LONGITUDE_HOURS =  -4.57905466667;
double LATITUDE_DEGREES =  76.53525;
double LONGITUDE_DEGREES =  -68.68582;
double SINLAT  = 0.907387685518;
double COSLAT  = 0.420294644471;
double LONGRAD  = -1.19879370843;
...
main(int argc, char *argv[])
{
	...
	double	sun_distance,jup_distance=0.0;
	int galileanMoon=0;
	...
	int newSourceFlag=0;
	double lstSet,lstRise,lstTransit,ha,har,lowel,phi;
	double utcSet,utcTransit,utcRise,lstd;
	double utcRiseS,utcSetS,utcTransitS;
	short dummy;
	int verboseFlag=0;
	int utcSetH,utcSetM,utcRiseH,utcRiseM,utcTransitH,utcTransitM;
 ...
 int              hours, minutes;	
	...
	double sunaz,sunel,jupaz=0.0,jupel=0.0;
...
/* for earthtilt and sidereal_time */
	...
	int gotn=0,gotoptical=0;
	double width;
        poptContext optCon;
        char poptc;

	  struct  poptOption optionsTable[] = {
                {"source",'s',POPT_ARG_STRING,&source,'s', "source name."},
                {"ra",'r',POPT_ARG_DOUBLE,&ra,'r',
                "RA in hours  (decimal hours)"},
                {"dec",'d',POPT_ARG_DOUBLE,&dec,'d',
                "DEC in degrees (decimal degrees)"},
                {"epoch",'e',POPT_ARG_DOUBLE,&epochd,'e',
                "Epoch of input coordinates 1950 or 2000."},
		{"time",'t',POPT_ARG_STRING,&timeString,'t',
		"date-time in double quotes: e.g: \"14 Dec 2010 12:40:15\""},
                {"optical",'o',POPT_ARG_NONE,0,'o',
                "Lookup optical catalog for nearby sources"},
                {"when",'w',POPT_ARG_NONE,0,'w',
                "flag for rise/set/transit times"},
                POPT_AUTOHELP
                {NULL,0,0,NULL,0},
		"Command to lookup a source's coordinates.  --help gives a detailed usage.\n"
        };
 ...
 if(argc<2) usage(-1,"Insufficient number of arguments","At least source- name required.");
  ...
  if((gotra==1)||(gotdec==1)) {
        ...
        "if coordinates are specified, source-name, ra, dec, epoch are all requi red; velocity is optional.\n");
                }
                else  {
  ...
  if(c<-1) {
	...
	if(!gottime) current_time=time(NULL);  	
	...
	if (sol_sys_flag == 0)
	{
            starcat(sname, &star_flag, &observe_source);
...
/* if the coordinates are B1950, precess them to J2000 first*/
	...
	if(!gottime) {
	    ...
	    hours = ts.tm_hour;
	    minutes = ts.tm_min;
	    seconds = (double) ts.tm_sec;

	hr = (double)hours + ((double)minutes)/60. + seconds/3600.;
	/* calculating julian day */
	/* note that tm_year and tm_yday are different from what we get
	 * using vme_sg_simple gps interface. tm_yday is from 0 to 365,
	 * and tm_year is years since 1900
	 */
	   tjd_upper = (double)((int)(365.25*((ts.tm_year+1900)+4711))) + 
			((double)ts.tm_yday+1.0) + 351.5;
	   tjd_lower=hr/24.;
	   tjd=tjd_upper+tjd_lower;
            ...
            ldpool_c ( "/home/glttest/catalogs/time.ker");
		...
		tjd_lower=tjd-tjd_upper;
               et2utc_c (  et , "C", 8, UTCLEN, utcstr );
		sscanf(utcstr,"%s %s %s %02d:%02d:%lf",junk,junk,junk,&utchour,
						&utcmin,&utcsec);
		hr=(double)utchour+(double)utcmin/60.+(double)utcsec;
	...
	*/
	...
	lowel=0.3491; /* 20 deg in radians */
	har=acos((sin(lowel)-sin(dec)*sin(phi))/(cos(dec)*cos(phi)));
	ha=12.*har/pi;
	lstTransit=ra/radian/15.0;
	lstRise=ra/radian/15.0-ha;
	lstSet=ra/radian/15.0+ha;
	lstd=hr-lst;
	utcRise=lstRise+lstd;
	utcTransit=lstTransit+lstd;
	utcSet=lstSet+lstd;
	if(utcRise<0.) utcRise=utcRise+24.;
	if(utcSet<0.) utcSet=utcSet+24.;
	if(utcTransit<0.) utcTransit=utcTransit+24.;
	hms(&utcRise,&utcRiseH,&utcRiseM,&utcRiseS,&dummy);
	hms(&utcSet,&utcSetH,&utcSetM,&utcSetS,&dummy);
	hms(&utcTransit,&utcTransitH,&utcTransitM,&utcTransitS,&dummy);
	

	   if((Planet.number==501)||(Planet.number==502)||(Planet.number==503)||(Planet.number==504)) {
	          ...
	          topo_planet(tjd, &Planet,&earth, delta, 
	        	&location, &ra, &dec, &distance,&radialVelocity);
	         	radialVelocity *= AUPERDAY_KMPERSEC;
                	    ra = ra * 15.0 * pi / 180.;
                	    dec = dec * pi / 180.;
           	    azelcal(&lst_radian, &ra, &dec, &jupaz, &jupel);
	           jup_distance= sunDistance(az,el,jupaz,jupel);
		galileanMoon=1;
	   }
...
if(galileanMoon==1) printf("%f %f %f %f\n",az,el, sun_distance, jup_distance*3600.);
else printf("%f %f %f\n",az,el, sun_distance);
	if(verboseFlag==1)
	printf("rising at %02d:%02d, transiting at %02d:%02d, setting at %02d:%02d UTC\n",
		utcRiseH,utcRiseM,utcTransitH,utcTransitM,utcSetH,utcSetM);

if(gotn==1) {
getNearbySources(lst_radian,az,el,width,gotoptical);
}
...
double tjd2et(double tjd)
{
  return((tjd-2451545.)* 86400.0+32.184+32.);
}
...
void usage(int exitcode, char *error, char *addl) {
        ...
        fprintf(stderr, "Usage: observe [options] --source <sourcename>\n"
                ...
                "--epoch -e <1950./2000.>\n"
                "  -h or --help    this help\n"
		"--time UTdate_time_string (e.g.: 4 Feb 2004 12:33:44) \n"
                "       (default: calculate for present moment)\n");
        exit(exitcode);
}
...
dec2=dec+width_radian;
	...
	"%d %s %d %d %f %d %d %f %f %f %f %s %f %s %s",
	&number, source_name, &rah, &ram, &ras, &decd, &decm, &decs,
  ...
  if(decsign==-1) {catdec=-catdec;}
	...
	if(optical==1) {
	fp = fopen("/home/glttest/catalogs/hipcat", "r");
    nsources=0;
    while ((end_of_file = fscanf(fp, 
	"%s %d %d %f %d %d %f %f %s",
