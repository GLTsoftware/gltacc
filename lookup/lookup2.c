...
/* for earthtilt and sidereal_time */
	...
	double width;
	  ...
	  struct  poptOption optionsTable[] = {
                ...
                " RA in hours  (decimal format"},
                {"dec",'d',POPT_ARG_DOUBLE,&dec,'d',
                "DEC in degrees (decimal "},
	   ...
	   if((Planet.number==501)||(Planet.number==502)||(Planet.number==503)||(Planet.number==504)) {
		   ...
		   Planet.number=5;
	          ...
	          topo_planet(tjd, &Planet,&earth, delta, 
	        	&location, &ra, &dec, &distance,&radialVelocity);
	         	radialVelocity *= AUPERDAY_KMPERSEC;
			if(prevra>ra) jup_distance_sign=1;
		        else jup_distance_sign=-1;
                	    ra = ra * 15.0 * pi / 180.;
                	    dec = dec * pi / 180.;
           	    azelcal(&lst_radian, &ra, &dec, &jupaz, &jupel);
	           jup_distance= (double)jup_distance_sign*sunDistance(az,el,jupaz,jupel);
		galileanMoon=1;
	   }
